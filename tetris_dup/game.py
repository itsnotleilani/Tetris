...
class Tetromino:
	...
	def next_move_down_collide(self, blocks, amount):
		collision_list = [block.down_collide(int(block.pos.y + amount), self.field_data) for block in self.blocks]
		return True if any(collision_list) else False
	...
	def move_down(self):
		if not self.next_move_down_collide(self.blocks, 1):
			for block in self.blocks:
				block.pos.y += 1
		else:
			for block in self.blocks:
